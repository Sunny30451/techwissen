  ...
  try {
    const result = await pool.query(`
      SELECT a.id, a.title, a.slug, a.excerpt, a.content_markdown,
             ...
             a.featured, a.published_at, a.updated_at,
             COALESCE(
               json_agg(json_build_object('name', t.name, 'slug', t.slug))
               FILTER (WHERE t.id IS NOT NULL), '[]'
             ) AS tags
  ...
  try {
    ...
    const result = await client.query(
      ...
      [article.title, article.slug, article.excerpt, article.contentMarkdown, article.categoryId,
        article.difficulty, article.readingTimeMinutes, article.featured, article.publishedAt]
    );
    ...
    const result = await client.query(
      `UPDATE articles
       SET title = $1, slug = $2, excerpt = $3, content_markdown = $4,
           ...
           featured = $8, published_at = $9, updated_at = NOW()
       WHERE id = $10
       RETURNING id`,
      [article.title, article.slug, article.excerpt, article.contentMarkdown, article.categoryId,
        article.difficulty, article.readingTimeMinutes, article.featured, article.publishedAt, articleId]
    );
...
app.delete('/api/admin/articles/:id', requireAdmin, async (req, res, next) => {
  try {
    const result = await pool.query('DELETE FROM articles WHERE id = $1 RETURNING id', [Number(req.params.id)]);
    if (!result.rows.length) return res.status(404).json({ error: 'Artikel nicht gefunden.' });
    res.status(204).end();
  } catch (error) {
    next(error);
