...
const articlePath = path.resolve(__dirname, '../content/hermes-agent-dokploy-contabo.md');
const packagePath = path.resolve(__dirname, '../packages/hermes-dokploy.zip');
...
const articleDefinition = {
  title: 'Hermes Agent mit Docker und Dokploy auf einem Contabo VPS bereitstellen',
  slug: 'hermes-agent-docker-dokploy-contabo',
  excerpt: 'Hermes Agent von Nous Research sicher und persistent per Dokploy betreiben – mit Cloud-LLM oder dem internen Ollama-Dokploy-Paket, optionalem Dashboard, OAuth und Backup-Konzept.',
  ...
  tags: [
    ...
    ['Docker', 'docker'], ['Dokploy', 'dokploy'], ['Contabo', 'contabo'],
    ['AI Agent', 'ai-agent'], ['Ollama', 'ollama'], ['LLM', 'llm'],
    ['Traefik', 'traefik'], ['Automation', 'automation'],
  ],
};
...
async function attachPackage(client, articleId) {
  ...
  await client.query(
    `UPDATE articles SET
       ...
       updated_at = NOW()
     WHERE id = $1`,
    [articleId, 'hermes-dokploy.zip', storageName, info.size, sha256],
  );
...
async function importArticle() {
  ...
  try {
    ...
    console.log(`Hermes-Artikel und Dokploy-Paket importiert: ${article.rows[0].slug}`);
  } catch (error) {
...
try {
  ...
  console.error('Hermes-Artikel konnte nicht importiert werden:', error);
  process.exitCode = 1;
} finally {
  await pool.end();
}
